...
* `currentState` is a array `[]int`, store the current temp combination (unfinished)

* `backtracking()` will return when `currentState` sum result >= problem asking

* If `currentState` sum result = problem asking, record this result into final collection

```
in the backtracking(... , i int)

For i := idx;  i < len(nums); i ++:

    currentState append nums[i]
    go deeper backtracking(), input i, not i+1
    undo currentState 
```
